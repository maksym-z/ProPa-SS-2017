  ...
  public Set<Zug> moeglicheZuege(boolean whiteToMove) {
     ...
     */
	  ...
	  for (Zug move: potentialMoves) {
		  ...
		  int emptyFieldIndex = feldIndex(move.lineFrom,move.columnFrom);
		  int occupiedFieldIndex = feldIndex(move.lineTo,move.columnTo);
		  
		  int kingFieldIndex = kingAt;
		  if (move.pieceId == 'K' && ((Figur)getFeld(move.lineFrom,move.columnFrom)).isWhite() == whiteToMove) {
			  kingFieldIndex = feldIndex(move.lineTo,move.columnTo);
		  } 
		  index = 0;
		  for (Feld field: felder) {
			  int line = index / 8 + 1;
